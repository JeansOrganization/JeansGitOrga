...
//筛选ID大于2，再以年龄分组，按年龄正向排序，获取前三位，输出匿名对象 包含年龄，数量，平均工资
var items = from li in list
            where li.Id > 2
            group li by li.Age into listGroup
            orderby listGroup.Key
            select new
            {
                age = listGroup.Key,
                count = listGroup.Count(),
                averSalary = listGroup.Average(t => t.Salary)
            };
foreach (var item in items)
{
    Console.WriteLine($"年龄:{item.age},人数：{item.count},平均工资:{item.averSalary}");
}