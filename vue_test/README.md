...
1. 作用：让样式在局部生效，防止冲突。
2. 写法：```<style scoped>```
注意: 当没有配置Scoped时,后引入的组件CSS覆盖先引入的组件CSS,父类会覆盖子类的CSS,当某个组件配置Scoped时则本组件CSS代码优先级更高
...
注:添加了replace路由跳转后会直接覆盖上一次的跳转记录，之后再操作浏览器的后退，会后退到上上次的跳转而不是上次的跳转(因为是直接覆盖而不是追加)
...
## npm语法
      运行vue：npm run serve
      安装less-loader：npm i less-loader@版本
      查看库全部版本：npm view less-loader versions
